...
beforeEach(() => {
  move = new Move();
  move.updatePveStats(fakeGameMaster.itemTemplates[4] as IPveMoveTemplate, 'Frenzy Plant');
  move.updatePvpStats(fakeGameMaster.itemTemplates[3] as unknown as IPvpMoveTemplate, 'Frenzy Plant');
});

test('should import all fields properly', () => {
  expect(move.id).toBe('FRENZY_PLANT');
  expect(move.type).toBe('GRASS');
  expect(move.name).toBe('Frenzy Plant');
